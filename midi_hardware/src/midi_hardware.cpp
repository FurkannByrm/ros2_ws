...
hardware_interface::CallbackReturn midi_hardware::MidiSystemHardware::on_init(const hardware_interface::HardwareInfo &info)
{
    ...
    for(const hardware_interface::ComponentInfo& joint : info_.joints)
    {
        ...
        m_JointsMap.at(joint.name).currentPosition = 0.0;
        m_JointsMap.at(joint.name).currentVelocity = 0.0;
        m_JointsMap.at(joint.name).targetPosition = 0.0;
        m_JointsMap.at(joint.name).targetVelocity = 0.0;
    }
...
hardware_interface::return_type midi_hardware::MidiSystemHardware::write(const rclcpp::Time &time, const rclcpp::Duration &period)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("MidiSystemHardware"), "Target velocities[rad/s]: %d, %d", m_JointsMap.at(m_HwParams.leftWheelName).targetVelocity,
 ...
 */    
    ...
    RCLCPP_INFO(rclcpp::get_logger("MidiSystemHardware"), "Target velocities[pwm] : %d, %d", pwms.left_pwm, pwms.right_pwm);
