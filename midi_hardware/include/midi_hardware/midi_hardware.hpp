...
namespace midi_hardware
{
  ...
  private:
        ...
        const std::pair<int32_t, int32_t> &rpms)
    {
      ...
      int32_t leftPWM, rightPWM = 0;

      int32_t leftRPM = abs(rpms.first);
      int32_t rightRPM = abs(rpms.second);
      leftPWM = 0 + (((255.0 - 0.0) / (1000.0 - 0.0))  * (leftRPM - 0.0));
      rightPWM = 0 + (((255.0 - 0.0) / (1000.0 - 0.0)) * (rightRPM - 0.0));
